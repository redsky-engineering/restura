...
### Allowed Characters (No Escaping)

The following characters can be used directly in unquoted values:
...
### Quoted String Syntax (Recommended)

For values containing commas, pipes, or parentheses, use quoted strings:
...
You can also escape special characters with backslashes in unquoted values:
...
### Quoted Strings in IN Operator

Mix quoted and unquoted values for complex cases:

**Example 5 - Filter:**

```
(status,in,"Active, Working"|Pending|"On Hold")
```
