...
<Aside type="caution" title="BIGINT Precision Warning">
	**BIGINT and BIGSERIAL are mapped to JavaScript `number`, which can lose precision for values outside ±9,007,199,254,740,991 (±2^53-1).** PostgreSQL BIGINT supports values up to ±9,223,372,036,854,775,807 (±2^63-1). For most applications using auto-incrementing IDs, this is not an issue until you exceed ~9 quadrillion records. However, if you're storing large integers from external systems or need guaranteed precision for all BIGINT values, be aware that precision loss can occur silently. Consider using DECIMAL/NUMERIC (returned as `string`) if you need to preserve exact values beyond JavaScript's safe integer range.
</Aside>
...
<Aside type="tip">
	For INTEGER vs BIGINT: Use `INTEGER` (max ~2.1 billion) for most auto-incrementing IDs unless you expect extremely high volume. Use `BIGINT` for tables that may exceed 2 billion records or when storing large integers from external systems. Be aware that BIGINT values beyond JavaScript's safe integer range (±2^53-1) may lose precision.
</Aside>
