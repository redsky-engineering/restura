...
import { RequestData, type ResturaSchema, RouteData } from '../schemas/resturaSchema.js';
...
export default function standardTypeValidationGenerator(currentSchema: ResturaSchema): ValidationDictionary {
	...
	for (const endpoint of currentSchema.endpoints) {
		for (const route of endpoint.routes) {
			if (!route.request || route.request.length === 0) continue;

			// Generate a unique key for this route's schema
...
function generateRouteKey(route: RouteData): string {
	return `${route.method}:${route.path}`;
}

function buildRouteSchema(requestParams: RequestData[]): Definition {
